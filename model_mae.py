...
class MaskedAutoencoder(nn.Module):
    ...
    # def __init__(self, d_numerical, categories, num_layers, hid_dim, factor = 4, bias = True, 
    #             decoder_depth=2, decoder_num_heads=4,
    #             norm_layer=nn.LayerNorm, norm_field_loss=False, encode_func='linear'):
        super().__init__()
    ...
    def random_masking(self, x, m, mask_ratio):
        ...
        if self.training:
            len_keep = int(L * (1 - mask_ratio))   # number of columns to keep 
        else:
        ...
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))  # gather missing ones
        
        # generate the binary mask: 0 is keep, 1 is remove
        mask = torch.ones([N, L], device=x.device)
        mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        mask = torch.gather(mask, dim=1, index=ids_restore)   # mask
        nask = torch.ones([N, L], device=x.device) - mask     # non-mask 
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        return x #, x_before_gather, x_after_gather


    def forward_loss(self, data, pred, mask, nask, observed_mask = None):
        """
        data: [N, 1, L]
        pred: [N, L]
    ...
    def forward(self, data, miss_idx, mask_ratio=0.5, full_cols = None):

        latent, mask, nask, ids_restore = self.forward_encoder(data, miss_idx, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)   # 0 ~ 1 
